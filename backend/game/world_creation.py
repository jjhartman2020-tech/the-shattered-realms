...
def _fallback_world(prompt: str) -> Dict:
    return {
        "name": "Untitled World", "premise": prompt, "genre": "custom",
        "era": "as described by the player", "tone": "adventure",
...
def generate_world(provider, prompt: str) -> Dict:
    ...
    if client is None or not model: return _fallback_world(prompt)
...
def _print_world(world: Dict) -> None:
    ...
    for label, key in [("FACTIONS", "factions"), ("IMPORTANT LOCATIONS", "important_locations"), ("CULTURES / SPECIES", "cultures_and_species"), ("COMMON GEAR", "common_weapons_and_gear"), ("MAJOR CONFLICTS", "major_conflicts"), ("SPECIAL RULES", "special_world_rules")]:
...
def run_world_creation(game_master) -> Dict:
    print("\n" + "=" * 52); print("WORLD CREATION"); print("=" * 52)
    ...
    if not request: request = "Create an original adventure world with a balanced mix of exploration, conflict, and mystery."
    while True:
        print("\nGenerating your world..."); world = generate_world(game_master.provider, request); _print_world(world)
        answer = input("\nDoes this match what you had in mind? (yes / describe changes): ").strip()
        if answer.lower() in {"yes", "y", "confirm", "looks good", "good"}: break
        if answer: request = f"ORIGINAL REQUEST:\n{request}\n\nPLAYER CHANGES:\n{answer}\nRegenerate the world incorporating the changes while preserving everything the player did not ask to change."
    ...
    # Character generation must see this exact confirmed world too. This controls
    # class flavor, abilities, starter kits, weapons, utilities, and special gear.
    from .world_character_generation import install_world_aware_character_generation
