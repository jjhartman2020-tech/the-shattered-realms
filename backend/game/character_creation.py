...
Treat the character's build as the gameplay identity. Every starter kit, weapon, utility item, special equipment option, and ability must support that identity and its strongest stats. Generate three useful variations of the SAME character concept rather than unrelated archetypes.
The setting changes how the build is expressed, but must never replace it with a setting stereotype. For example, a ninja/stealth build in a high-tech space setting should receive setting-native stealth blades, smart throwing weapons, infiltration tools, compact mobility gear, or similar ninja equipment—not a standard blaster loadout unless the player's build explicitly asks for guns. Match weapon attack_attribute to the stats and playstyle the player actually chose.
