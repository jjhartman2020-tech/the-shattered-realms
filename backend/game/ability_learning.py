...
def highest_unlocked_tier(level: int) -> str:
    ...
    for tier in TIER_ORDER:
        if level >= ABILITY_TIER_LEVELS[tier]:
            unlocked = tier
    return unlocked
...
def _fallback_library(player: Dict) -> List[Dict]:
    """Deterministic backup choices if live AI generation is unavailable."""
    level = int(player.get("level", 1) or 1)
    available_tiers = [tier for tier in TIER_ORDER if tier_is_unlocked(level, tier)]
    names = {
        "beginner": ("Driving Blow", "Quick Guard"),
        "novice": ("Relentless Rush", "Focused Burst"),
        "expert": ("Crushing Arc", "Phantom Advance"),
        "master": ("Dominion Strike", "Impossible Step"),
    ...
    for tier in available_tiers:
        ...
        for index, name in enumerate(names[tier]):
            ability = {
                "name": name,
                "description": f"A {tier} technique shaped by the character's class.",
                "type": "active",
                "category": "offensive" if index == 0 else "mobility",
                "tier": tier,
                "ability_point_cost": ABILITY_TIER_COSTS[tier],
                "resource": "class",
                "resource_cost": base_cost,
                "target": "enemy" if index == 0 else "self",
                "range": 1 if index == 0 else 0,
            }
            if index == 0:
                ability.update({"damage": {"beginner":"1d6","novice":"1d8","expert":"2d6","master":"3d6","legendary":"4d8"}[tier], "attack_attribute":"strength", "requires_attack_roll":True})
            else:
                ability.update({"movement_squares": {"beginner":2,"novice":3,"expert":5,"master":7,"legendary":10}[tier], "requires_attack_roll":False})
...
def _normalize_generated(raw: List[Dict], level: int) -> List[Dict]:
    ...
    for entry in raw:
        ...
        ability["type"] = str(ability.get("type") or "active").lower()
        if ability["type"] != "active":
            ability["type"] = "active"
        ability["resource"] = "class"
        ability["resource_cost"] = max(5, int(round(max(5, int(ability.get("resource_cost", _tier_resource_cost(tier)) or _tier_resource_cost(tier))) / 5.0)) * 5)
...
def generate_ability_choices(provider, player: Dict) -> List[Dict]:
    """Generate six class-specific abilities from only the tiers unlocked by level."""
    level = int(player.get("level", 1) or 1)
    client = getattr(provider, "client", None)
    model = getattr(provider, "model", None)
    if client is None or not model:
        return _fallback_library(player)
    available_tiers = [tier for tier in TIER_ORDER if tier_is_unlocked(level, tier)]
...
Only use tiers listed in unlocked_tiers. Tier AP costs are fixed by the game and must be: Beginner=1, Novice=3, Expert=6, Master=10, Legendary=15.
...
Every ability must include: name, description, type='active', category, tier, resource_cost, target, range, requires_attack_roll, and exact mechanics. Include damage, healing, movement_squares, shield, duration, target_count, status effect, or other exact numeric effect whenever applicable.
Resource costs must be positive multiples of 5. A generated ability may cost more Resource than the player's current maximum; that is allowed. The player can learn/equip it but cannot use it until they can pay the full Resource cost.
Do not use cooldowns. Do not create basic weapon attacks; these are abilities."""
    payload = {
        "name": player.get("name"), "class": player.get("class"), "backstory": player.get("background"),
...
def available_to_learn(player: Dict, abilities: List[Dict]) -> List[Dict]:
    ...
    for raw in abilities:
        if not isinstance(raw, dict):
            continue
        ability = deepcopy(raw)
        tier = normalize_tier(ability.get("tier"))
        ability["tier"] = tier
        if str(ability.get("name", "")).strip().lower() in known or not tier_is_unlocked(level, tier):
            continue
...
def learn_ability(player: Dict, ability: Dict, forget_index: int | None = None) -> Dict:
    ...
    if ap < cost:
        raise ValueError(f"You need {cost} AP but only have {ap}")
    ...
    learned = deepcopy(ability)
    learned["tier"] = tier
    learned["ability_point_cost"] = cost
...
def _effect_text(ability: Dict, resource_name: str) -> str:
    ...
    for key, label in (("damage", "Damage"), ("healing", "Heal"), ("shield", "Shield"), ("movement_squares", "Move")):
        value = ability.get(key)
        if value not in (None, "", 0, "0"):
            parts.append(f"{label} {value}" + (" squares" if key == "movement_squares" else ""))
    ...
    if ability.get("duration") not in (None, "", 0, "0"):
        parts.append(f"Duration {ability.get('duration')}")
    if ability.get("effect"):
        parts.append(str(ability.get("effect")))
    return " | ".join(parts)


def run_ap_spending_screen(game_master) -> Dict:
    """Browse and optionally buy one or more abilities. Leaving spends nothing and AP is stored forever."""
    state = game_master.state
    player = state.data.setdefault("player", {})
    if state.data.get("combat", {}).get("active"):
        print("You cannot learn or forget abilities during combat.")
        return deepcopy(player)
    if not player.get("character_creation_complete"):
        print("Finish character creation before learning new abilities.")
        return deepcopy(player)
    ...
    while True:
        ...
        for i, ability in enumerate(known, 1):
            print(f"  {i}. {ability.get('name')} [{_effect_text(ability, resource_name)}]")
        ...
        if not choices:
            print("No new abilities were generated right now. Your AP remains saved.")
            return deepcopy(player)
        print("AVAILABLE ABILITIES")
        for i, ability in enumerate(choices, 1):
            afford = "" if ap >= ability_ap_cost(ability) else " [NOT ENOUGH AP YET]"
            resource_warning = ""
            if int(ability.get("resource_cost", 0) or 0) > int(player.get("max_resource", 0) or 0):
                resource_warning = " [RESOURCE TOO LOW TO USE YET]"
            print(f"{i}. {ability.get('name')} — {ability.get('description', '')} [{_effect_text(ability, resource_name)}]{afford}{resource_warning}")
        answer = input("\nChoose an ability number, 'refresh' for new choices, or 'leave' to save AP: ").strip().lower()
        if answer in {"leave", "exit", "back", "done", "save"}:
            state.save()
            print(f"Saved {int(player.get('ability_points', 0) or 0)} AP for later.")
            return deepcopy(player)
        if answer == "refresh":
            continue
        try:
        ...
        cost = ability_ap_cost(selected)
        if ap < cost:
            print(f"You only have {ap} AP. {selected.get('name')} costs {cost} AP. Nothing was spent.")
            continue
        ...
        if len(known) >= MAX_ABILITY_SLOTS:
            ...
            for i, ability in enumerate(known, 1):
                print(f"  {i}. {ability.get('name')}")
            raw = input("Choose the ability to forget, or type 'cancel': ").strip().lower()
            if raw == "cancel":
                continue
            try:
                forget_index = int(raw) - 1
            except ValueError:
                forget_index = -1
            if not 0 <= forget_index < len(known):
        ...
        result = learn_ability(player, selected, forget_index)
        state.save()
        print(f"\nLearned {result['learned'].get('name')}! AP remaining: {result['ability_points_remaining']}")
        if result.get("forgotten"):
            print(f"Forgot {result['forgotten'].get('name')} permanently.")
        if int(player.get("ability_points", 0) or 0) <= 0:
            return deepcopy(player)
